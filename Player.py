...
class Player:
    rounds_played = 0
    rounds = []
    # A handicap is calculated from the avg index's of your most recent 20 rounds
    handicap_rounds = []
    cap = 0
    avg_putts = 0
...
=======
    # Sort algorithm to find top k rounds for the selected stat
    def calculate_top_x(self, k, stat):
        if (self.get_rounds_played() < k):
>>>>>>> 416bd4bae51ee694a179a5606e3b94bbedf10a31
            print("Not enough rounds")
            return -1

    #k is size of window or the length of the streak, type is best/worst window, stat is the stat to measure
    def sliding_window(self, k, type, stat):
        if (self.get_rounds_played() < k):
            print("Not enough rounds")
            return -1
        # take the first k elements and store a copy
        best_streak = self.rounds[:k].copy()
        # create a copy to iterate over *I think I can just use reference here*
        ...
        for r in self.rounds[k:]:  #start moving the window
            ...
            if (stat == "score"):
                sum_stat = sum_stat - current_window[0].get_score(
                ) + r.get_score()
                if (type):
                    if (sum_stat <= max_sum_stat):
                        max_sum_stat = sum_stat
                        # make a copy of the new window of rounds we want as conditions were met
                ...
                sum_stat = sum_stat - current_window[0].calcualte_index(
                ) + r.calculate_index()
                if (type):
                    if (sum_stat <= max_sum_stat):
                        max_sum_stat = sum_stat
                ...
                sum_stat = sum_stat - current_window[0].get_putts(
                ) + r.get_putts()
                if (type):
                    if (sum_stat <= max_sum_stat):
                        max_sum_stat = sum_stat
    ...
    def add_round(self, round):
        ...
        # implelement a queue to keep track of the LATEST 20 rounds
        if len(self.handicap_rounds) > 19:
